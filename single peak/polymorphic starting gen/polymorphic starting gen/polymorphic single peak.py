...
#changing the fitness landscape for this replication
...
#populating gef array of first generation of this replication with polymorphic genome, epigenome, and the correspong fitness (preliminarily NoneType)
gef = [[random.choice(ref_types), random.choice(ref_types), None] for i in range(popn)]

for i1 in range(popn): #makingchanges to fitness of the individual on the basis of genome and epigenome
    gef[i1][3] = max(gfitnessref[ref_types.index(gef[i1][0])], efitnessref[ref_types.index(gef_transient[i1][1])]) 
...
for i in range(total_gen-1000):
    ...
    gm_forward += [geomean(avg_w[0:(999+i)])]
    gm_backward += [geomean(avg_w[(8999-i):])]

gm_backward = list(reversed(gm_backward))

plt.figure(figsize=(45, 20))
plt.plot(generations, avg_w, 'r', generations[1000:], gm_forward, 'b', generations[1000:], generations[1000:], gm_backward, 'y')
plt.savefig('monomorphic, single peak, forwards+backwards.png', bbox_inches = 'tight')

with open('avg_fitnesses.csv', 'w') as myfile:
    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    wr.writerow(avg_w)

with open('gm_forward.csv', 'w') as myfile:
    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
